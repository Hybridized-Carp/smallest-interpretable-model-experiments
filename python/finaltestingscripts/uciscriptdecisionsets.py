...
########
########
######## You have already partially fucked this file up, you are stupid but try not to beat yourself up too much
########
########
# does it make it better or worse that you were a little drunk when you did it?
# do you think you would have made the same mistake if you were sober (yes)
# are you just stupid, yes
# you have the innate capacity to learn from your mistakes though!
# -> use version control next time <3
...
def FindStrictExtStr(classification_instance, size, model, annotations, example , expres):
    ...
    if model[1] != expres:
        ...
        hmd = (ex2^example)
        while hmd >bv.bitarray(0)):
            if (hmd%2 == 1):
                nA = annotations.copy()
                t=np.full(hmd.shape, False)
                t[findex] = True
...
def mapnparraytobitvector(nparry):
    nl=[]
    for i in nparry:
        bv=ba.bitarray()
        bv.pack(i.tobytes())
...
if (rt):
    ...
    #print(xhnpy)
    a = FindOptModelStr(list(zip(mapnparraytobitvector(xhnpy),yhnpy)),2**4)
else:
