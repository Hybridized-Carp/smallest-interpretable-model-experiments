...
namespace model {
    ...
    bool testNDSM(const vector<dataset::datapoint> Ci, const ndsm& model, dataset::datapoint &miscex) {
        for (dataset::datapoint example : Ci) {
            for (int j = 0; j < model.terms.size(); j++) {
                ...
                if (!rt && (example.result == model.defaultRule)) {
                    return false;
                }
            }
            if (example.result != model.defaultRule) {
                return false;
            }
        }
        return true;
